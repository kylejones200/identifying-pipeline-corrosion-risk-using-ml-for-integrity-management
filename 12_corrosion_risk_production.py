    ...
    df = pd.DataFrame(
        {
            ...
            "soil_resistivity": rng.normal(3000, 800, n_joints).clip(
                200, 8000
            ),  # ohm-cm
            ...
            "ili_metal_loss": rng.beta(1.5, 10, n_joints)
            * 100,  # percent wall thickness
        }
    )
...
def prepare_features(df):
    ...
    logger.info(
        f"  Categorical features ({len(categorical_cols)}): {', '.join(categorical_cols)}"
    )
...
def train_corrosion_risk_model(X, y, numeric_cols, categorical_cols):
    ...
    logger.error(
        f"  Positive class (failures) in test: {y_test.sum()} ({y_test.mean():.1%})"
    )
...
def create_work_list(model, X_test, y_test, y_pred_proba, budget_joints=50):
    ...
    # Create work list
    work_list = risk_df.sort_values("value_per_dollar", ascending=False).head(
        budget_joints
    )
...
def analyze_business_value(work_list, risk_df, y_test):
    ...
    # Calculate lift
    ml_lift_vs_ili = (
        ((ml_captured - ili_captured) / ili_captured * 100) if ili_captured > 0 else 0
    )
    ml_lift_vs_age = (
        ((ml_captured - age_captured) / age_captured * 100) if age_captured > 0 else 0
    )
    ml_lift_vs_random = (
        ((ml_captured - random_captured) / random_captured * 100)
        if random_captured > 0
        else 0
    )
...
def main():
    ...
    importance_df = analyze_feature_importance(
        model, X_test, numeric_cols, categorical_cols
    )
